...
CREATE TABLE IF NOT EXISTS grades (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  produto_catalogo_id UUID NOT NULL REFERENCES produtos_catalogo(id) ON DELETE CASCADE,
  cor TEXT,
  tamanho TEXT,
  estoque INTEGER NOT NULL DEFAULT 0
);

CREATE TABLE IF NOT EXISTS ordenacao_manual (
...
CREATE INDEX IF NOT EXISTS idx_grades_produto_catalogo_id ON grades(produto_catalogo_id);
...
async function createTablesAndSeed() {
  ...
  try {
    ...
    // Adiciona a coluna senha_hash se ainda não existir
    await client.query(`
      DO $$
      BEGIN
        IF NOT EXISTS (
